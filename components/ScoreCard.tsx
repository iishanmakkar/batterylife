...
export default function ScoreCard({ report, health }: { report: BatteryReport; health: HealthAnalysis }) {
  ...
  return (
    <div className="bg-[var(--color-bg2)] border border-[var(--color-border)] rounded-[20px] p-6 card-glow">
      <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 items-center">
        ...
        <div className="relative w-[180px] h-[180px] mx-auto sm:mx-0 shrink-0">
          <svg width="180" height="180" viewBox="0 0 180 180" aria-hidden="true">
            <circle cx="90" cy="90" r="82" fill="none" stroke="var(--color-border)" strokeWidth="10" />
            <motion.circle
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="font-[family-name:var(--font-mono)] text-[44px] font-bold leading-none" style={{ color: health.color }}>
              {hasData ? health.score : '?'}
            </span>
            <span className="text-sm text-[var(--color-text3)] mt-0.5">{hasData ? '/100' : ''}</span>
          </div>
        </div>

        {/* Info */}
        <div className="text-center sm:text-left">
          <h2 className="font-[family-name:var(--font-syne)] text-[28px] font-extrabold tracking-tight mb-1.5">
            {hasData ? `${health.status} Health` : 'Incomplete Data'}
            {hasData && (
              <span className="text-lg opacity-50 font-normal ml-2">· Grade {health.grade}</span>
            )}
          </h2>
          <p className="text-sm text-[var(--color-text2)] mb-3">
            {report.device.name}

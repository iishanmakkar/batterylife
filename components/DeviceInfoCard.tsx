  ...
  return (
    <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-2xl p-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {fields.map((f, i) => (
          <div key={i}>
            <div className="text-[11px] text-[var(--text3)] uppercase tracking-[1px] font-medium mb-1">{f.label}</div>
            <div className="font-mono text-[13px] text-[var(--text1)] font-medium">{f.value}</div>
          </div>
        ))}
      </div>
    </div>

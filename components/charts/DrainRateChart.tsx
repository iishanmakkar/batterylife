...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: RatePoint }> }) => {
  ...
  return (
    <div style={{ background: 'var(--tooltip-bg)', border: '1px solid var(--tooltip-border)', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
      ...
      <p className="font-mono" style={{ color: 'var(--tx1)' }}>{(d.rate / 1000).toFixed(1)} W average draw</p>
      <p style={{ color: 'var(--chart-muted)' }}>{d.wh.toFixed(1)} Wh used over {d.dur}</p>
    </div>
  );
};
    ...
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--bdr)', borderRadius: 16, padding: 20 }}>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
          ...
          <Line yAxisId="right" type="monotone" dataKey="rate" stroke="#ffb830" strokeWidth={2} dot={{ r: 3, fill: '#ffb830', stroke: 'var(--chart-dot-stroke)', strokeWidth: 2 }} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
